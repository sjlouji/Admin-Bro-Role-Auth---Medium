...
const index_1 = require("./form-group/index");
const css_class_1 = require("../utils/css-class");
const date_utils_1 = require("../utils/date-utils");
const StyledDatePicker = styled_components_1.default(index_1.InputGroup) `
...
const DatePicker = (props) => {
    ...
    if (value && value.constructor.name !== 'Date') {
        ...
        if (dateNum) {
            ...
            stringValue = date_utils_1.formatDateProperty(dateValue, propertyType);
        }
    }
    else if (value && value.constructor.name === 'Date') {
        stringValue = date_utils_1.formatDateProperty(value, propertyType);
    }
    const onDatePickerChange = (date) => {
        if (!disabled) {
            onChange(date_utils_1.formatDateProperty(date, propertyType));
        }
    };
    return (react_1.default.createElement(react_1.default.Fragment, null,
        react_1.default.createElement(Overlay, { onClick: () => setHidden(true), className: hidden ? 'hidden' : 'visible' }),
        react_1.default.createElement(StyledDatePicker, { className: css_class_1.cssClass('DatePicker', hidden ? 'normal' : 'active') },
            react_1.default.createElement(input_1.Input, { value: stringValue || '', onChange: (event) => onChange(event.target.value), onFocus: () => setHidden(false), disabled: disabled }),
            react_1.default.createElement(button_1.Button, { variant: "primary", type: "button", size: "icon", disabled: disabled, onClick: () => setHidden(!hidden) },
                react_1.default.createElement(icon_1.Icon, { icon: "Calendar" })),
            !hidden ? (react_1.default.createElement(DatePickerWrapper, null,
                react_1.default.createElement(react_datepicker_1.default, Object.assign({ selected: dateValue, onChange: onDatePickerChange, inline: true, showTimeInput: propertyType === 'datetime' }, other)))) : '')));
};
